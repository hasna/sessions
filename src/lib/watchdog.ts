...
/* ── Watchdog scanning ─────────────────────────────────────────────────────
 * Detection is separate from the restart path above: it classifies a pane as
 * dead or crashed and reports it, without acting. It reads a narrower tmux
 * format of its own (WATCHDOG_SCAN_TMUX_FORMAT) because it keys on
 * window_name to identify a session's primary window, which the restart
...
const WATCHDOG_SCAN_TMUX_FORMAT = [
  "#{session_name}",
  "#{window_name}",
  "#{window_index}",
  "#{pane_index}",
...
export function scanWatchdogSessions(options: WatchdogScanOptions = {}): WatchdogCandidate[] {
  const runner = options.runner ?? createWatchdogTmuxRunner(options.tmuxBin);
  const result = runner(["list-panes", "-a", "-F", WATCHDOG_SCAN_TMUX_FORMAT]);
